...
namespace _7D___Quistis.ChoiceProvider
{
    internal class DiscordChoiceProviderGetAllMatches : IChoiceProvider
    {
        ...
        private async Task GetAllMatches()
        {
            ...
            {
                if (tournament.tournament.state.ToString() == "underway" || tournament.tournament.state.ToString() == "group_stages_underway")
                {
                    string matchesjson = await ConnectionChallongeAPI.GetMatches(tournament.tournament.name);
                    string participantsJson = await ConnectionChallongeAPI.GetParticipant(tournament.tournament.name);
